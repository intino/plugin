...
import static io.intino.plugin.lang.parser.TaraParserUtil.*;
...
public class TaraParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // doc? signature body?
  public static boolean mogram(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogram")) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, MOGRAM, "<mogram>");
    r = mogram_0(b, l + 1);
    r = r && signature(b, l + 1);
    p = r; // pin = 2
    r = r && mogram_2(b, l + 1);
  ...
  // doc?
  private static boolean mogram_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogram_0")) return false;
    doc(b, l + 1);
    return true;
  }

  // body?
  private static boolean mogram_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogram_2")) return false;
    body(b, l + 1);
    return true;
  }
  ...
  public static boolean mogramReference(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogramReference")) return false;
    if (!nextTokenIs(b, HAS)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, MOGRAM_REFERENCE, null);
    r = consumeToken(b, HAS);
    p = r; // pin = 1
    r = r && report_error_(b, mogramReference_1(b, l + 1));
    r = p && report_error_(b, identifierReference(b, l + 1)) && r;
    r = p && mogramReference_3(b, l + 1) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }

  // ruleContainer*
  private static boolean mogramReference_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogramReference_1")) return false;
    while (true) {
      int c = current_position_(b);
      if (!ruleContainer(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "mogramReference_1", c)) break;
    }
    return true;
  }

  // tags?
  private static boolean mogramReference_3(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "mogramReference_3")) return false;
    tags(b, l + 1);
    return true;
  }

  /* ********************************************************** */
  // varInit | variable | mogram | mogramReference
  ...
  /* ********************************************************** */
  // COMMENT? NEWLINE* (dslDeclaration NEWLINE+)? imports? (mogram NEWLINE+)*
  ...
  /* ********************************************************** */
  // (subNode | (metaIdentifier ruleContainer* parameters? identifier? facets? parent?)) (constraint? tags?)
