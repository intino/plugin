...
public class ServiceTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(trigger("dependencies")).output(literal(":/opt/")).output(placeholder("artifact", "lowercase")).output(literal("/")).output(expression().output(placeholder("directory")).next(expression().output(literal("dependency")))).output(literal("/*")));
		return rules;
	}
