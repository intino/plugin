...
public class DecorableTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
				...
				rule().condition((type("node"))).output(literal("public ")).output(expression().output(mark("inner"))).output(literal(" ")).output(expression().output(mark("abstract"))).output(literal(" class ")).output(mark("name", "FirstUpperCase")).output(literal(" extends Abstract")).output(mark("name", "FirstUpperCase")).output(literal(" {\n\tpublic ")).output(mark("name", "FirstUpperCase")).output(literal("(io.intino.magritte.framework.Node node) {\n\t\tsuper(node);\n\t}\n\n\t")).output(expression().output(mark("node").multiple("\n\n"))).output(literal("\n}")),
				rule().condition((type("nodeGen"))).output(literal("public ")).output(expression().output(mark("inner"))).output(literal(" abstract class Abstract")).output(mark("name", "FirstUpperCase")).output(literal(" extends io.intino.magritte.framework.Layer {\n\tpublic Abstract")).output(mark("name", "FirstUpperCase")).output(literal("(io.intino.magritte.framework.Node node) {\n\t\tsuper(node);\n\t}\n\n\t")).output(expression().output(mark("nodeGen").multiple("\n\n"))).output(literal("\n}"))
		);
	}
}