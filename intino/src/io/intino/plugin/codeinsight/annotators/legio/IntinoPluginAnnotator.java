...
import io.intino.plugin.codeinsight.annotators.legio.analyzers.IntinoPluginAnalyzer;
...
public class IntinoPluginAnnotator extends TaraAnnotator {
	@Override
	public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
		if (element instanceof Mogram && element.getContainingFile().getName().endsWith("." + LegioFileType.instance().getDefaultExtension()) &&
			...
			analyzeAndAnnotate(holder, new IntinoPluginAnalyzer((Mogram) element, (ArtifactLegioConfiguration) configuration));
		}
	}

	private boolean isDataHub(Mogram mogram) {
		return mogram != null && (mogram.type().equals("IntinoPlugin") || mogram.type().equals("Artifact.IntinoPlugin"));
	}
}